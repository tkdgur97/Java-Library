...
public class BookUpdateSearch_admin implements ActionListener, MouseListener {
	private JPanel jp;
	...
	public BookUpdateSearch_admin() throws Exception {
		// 패널만들기
		jp = new JPanel();
...
//		infoTable.getTableHeader().setResizingAllowed(false); //크기 조절 불가
			...
			for (int i = 0; i < bookInfo.size(); i++) {
				...
				if (dto.getTitle().equals(infoTable.getValueAt(a, 1))) {
					if (dto.getRent().equals("Y")) {
						...
						int c =JOptionPane.showOptionDialog(null, "내용", "제목", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, buttons, "두번째값");
	...
	public static void main(String[] args) throws Exception {
		...
		BookUpdateSearch_admin a = new BookUpdateSearch_admin();
